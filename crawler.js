...
async function getSearchURL(nick){
    ...
    return "https://maplestory.nexon.com"+editerPick.find("td.left > dl > dt > a").attr("href");
}

async function getinfoURL(nick){
    const ChracterURL = await getSearchURL(nick);
...
async function getRankingInfo(nick){
    const infoURL = await getinfoURL(nick);
...
async function get2(){
    const url = "https://goongam.github.io/note/";
